...
impl<T: Copy> Alias<T> {
    ...
    pub fn from_weighted_tuples(items: &[WeightedTuple<T>]) -> Alias<T> {
        ...
        let sum = items.iter().map(|wt| wt.weight).fold(0, |total, next| total + next);
        let normalized_weight_tuples = items.iter().map(|wt| {
            NormalizedWeightTuple {
        ...
        while !small_items.is_empty() && !large_items.is_empty() {
            ...
            let reduced_fraction = current_large_item.fractional_weight + current_large_item.fractional_weight - one;
...
mod tests {
    ...
    fn getting_familiarity() {
        ...
        assert_eq!(format!("{:?}", tuple), String::from("WeightedTuple { weight: 123, value: \"Paul Freakn Baker\" }"));
    }
    ...
    fn can_we_use_fractions() {
        ...
        let fractions: Vec<Fraction> = items.iter().map(|&numerator| Fraction::new(numerator, sum)).collect();
